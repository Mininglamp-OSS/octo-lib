...
// 	resp, err := network.Post(c.cfg.IMExtendURL+"/message/delete", []byte(util.ToJson(req)), c.wkIMManagerTokenHeader())
// 	if err != nil {
// 		return err
// 	}
// 	return c.handlerIMError(resp)
...
// wkIMManagerTokenHeader 返回 WuKongIM 管理 API 的认证 header
func (c *Context) wkIMManagerTokenHeader() map[string]string {
	if c.cfg.WuKongIM.ManagerToken == "" {
		return nil
	}
	return map[string]string{"token": c.cfg.WuKongIM.ManagerToken}
}

// WuKongIMManagerTokenHeaderMap 返回 WuKongIM 管理 API 的认证 header (公共方法供外部模块使用)
func (c *Config) WuKongIMManagerTokenHeaderMap() map[string]string {
	if c.WuKongIM.ManagerToken == "" {
		return nil
	}
	return map[string]string{"token": c.WuKongIM.ManagerToken}
}
